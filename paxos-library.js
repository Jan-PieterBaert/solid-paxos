...
export async function getPaxosMessage(content, url) {
  ...
  for (const content of json_ld) {
    ...
    if (retval.type === "Accept") {
      retval["value"] = data["http://www.semanticweb.org/jan-pieter/ontologies/2023/2/paxos-ontology#value"].map(
        (i) => i["@value"]
      )[0];
    }
    if (retval.type === "Accepted") {
      retval["value"] = data["http://www.semanticweb.org/jan-pieter/ontologies/2023/2/paxos-ontology#value"].map(
        (i) => i["@value"]
      )[0];
      retval["accepted"] = data[
        "http://www.semanticweb.org/jan-pieter/ontologies/2023/2/paxos-ontology#acceptedStatus"
      ].map((i) => i["@value"])[0];
    }
    if (retval.type === "Ack") {
      retval["ackKey"] = data["http://www.semanticweb.org/jan-pieter/ontologies/2023/2/paxos-ontology#ackKey"].map(
        (i) => i["@value"]
      )[0];
    }
    retvals.push(retval);
...
export async function processInboxElem(element) {
  ...
  let data = JSON.parse(localStorage.getItem(key)) || { lastPromised: -Infinity };

  console.log("Inbox element", element);

  // below is option 2
  if (Number(element.round) < Number(data.lastPromised)) {
    console.log("Inbox item has a round that can't be promised to anymore, ignoring");
    console.log(`element is: ${JSON.stringify(element)}, saved data is ${JSON.stringify(data)}`);
    deleteInboxElem(element.url).then(() => {
      console.log(`Deleted item with url: ${element.url}`);
    });
    // TODO: send nack
    return false;
  }
...
async function sendAck(element) {
  ...
  await sendElement({ data: dataToSend, url: element.from });
}
...
export async function sendPrepare(element) {
  ...
  if (Number(data.lastPromised) < Number(element.round)) {
    ...
    for (const acceptorUrl of element.inboxArray) {
      outbox[`${acceptorUrl}|${key}|${data.lastAction}`] = { data: dataToSend, url: acceptorUrl };
    }
  }
}
export async function answerToPrepare(element) {
  ...
  let data = JSON.parse(localStorage.getItem(key)) || { lastPromised: -Infinity };
  if (Number(data.lastPromised) <= Number(element.round)) {
    console.log(`Sending promise back to ${JSON.stringify(element)}`);
    let dataToSend = {
      "@context": [
        "https://www.w3.org/ns/activitystreams",
        { paxos: "http://www.semanticweb.org/jan-pieter/ontologies/2023/2/paxos-ontology#" },
    ...
    outbox[`${element.proposer}|${key}|${data.lastAction}`] = { data: dataToSend, url: element.proposer };
    localStorage.setItem(key, JSON.stringify(data));
  }
...
export async function answerToPromise(element) {
  ...
  let neededAcceptors = (data || { acceptors: [] })["acceptors"] || [];
  let acceptAllowed = neededAcceptors.every((elem) => haveAcceptors.includes(elem));
  if (acceptAllowed) {
    console.log(`Sending accept back to ${JSON.stringify(element)}`);
    ...
    console.log("OUTBOX ELEMENT", dataToSend);
...
export async function processAccept(element) {
  console.log("processing accept", element);
...
export async function createAccepted(element, accepted) {
  console.log("CREATED ACCEPTED MESSAGE WITH VALUE", element.value);
...
export async function answerToAccepted(element) {
  ...
  let data = JSON.parse(localStorage.getItem(`${element.id}|${element.instance}`)) || { lastPromised: -Infinity };
...
export function inboxCron() {
  if (!inboxCronStarted) {
    console.log("INBOX CRON STARTED");
...
async function checkInboxElements() {
  console.log("Checking inbox elements");
  const inboxUrl = localStorage.getItem("inbox");
  if (inboxUrl === undefined) {
    console.log("Can't check inbox, localstorage item not set");
  } else {
...
export function outboxCron() {
  if (!outboxCronStarted) {
    console.log("OUTBOX CRON STARTED");
    // TODO: fix cronjob timing
    outboxJob = cron.job(
      "*/30 * * * * *",
      async function () {
...
async function sendOutboxElements() {
  console.log("Sending all outbox elements");
  console.log(`Outbox is`);
  console.log(outbox);
